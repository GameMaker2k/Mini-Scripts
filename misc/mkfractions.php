...
function mkfraction($numerator, $denominator, $number = null, $altver = false, $fiximproper = true, $simplify = false)
{
    ...
    if ($fiximproper !== true && $fiximproper !== false) {
        $fiximproper = true;
    }
    if ($simplify !== true && $simplify !== false) {
        $simplify = false;
    }
    if (($numerator === 0 || $numerator === null) && ($denominator === 0 || $denominator === null) & ($number !== null && $number !== 0)) {
        ...
        $number = (int) $getnum[0];
        $getfrac = explode("/", $getnum[1]);
        $numerator = (int) $getfrac[0];
        $denominator = (int) $getfrac[1];
    }
    ...
    if (is_numeric($numerator) && is_string($numerator)) {
        $numerator = (int) $numerator;
    }
    if (!is_numeric($denominator)) {
        $denominator = 0;
    }
    if (is_numeric($denominator) && is_string($denominator)) {
        $denominator = (int) $denominator;
    }
    if ((int) $denominator === 0 && $number === 0) {
        $returnvar = "<span style=\"font-size: 16px;\">".$number."</span>";
        return $returnvar;
    }
    if (!is_numeric($number)) {
        $number = null;
    ...
    if ($numerator > 0) {
        ...
        if ($altver === true) {
            $returnvar .= "<span style=\"font-size: 16px;\">&frasl;</span>";
        }
        if ($altver === false) {
            $returnvar .= "<span style=\"font-size: 16px;\">/</span>";
        }
