...
if(!isset($_SESSION['server'])) { $_GET['act'] = "login"; }
if(!isset($_SESSION['user'])) { $_GET['act'] = "login"; }
if(!isset($_SESSION['password'])) { $_GET['act'] = "login"; } }
?>
...
if($chck_ftp_pwd=="/") { $myftpdir = $chck_ftp_pwd; }
if($chck_ftp_pwd!="/") { $myftpdir = $chck_ftp_pwd."/"; }
$filelist = ftp_rawlist($ftp_id, ".");
$i = 0; $num = count($filelist);
$flist = array(); $fli = 0; $dlist = array(); $dli = 0;
while($i < $num) {
$info = preg_split("/[\s]+/", $filelist[$i], 9);  
if(preg_match("/^d/", $info[0])&&$info[8]!="."&&$info[8]!="..") { 
	$dlist[$dli]['name'] = $info[8]; ++$dli; }
if(preg_match("/^\-/", $info[0])) {
	$flist[$fli]['name'] = $info[8]; ++$fli; }
++$i; }
if($chck_ftp_pwd!="/") { echo "<a href=\"ftp.php?act=list&amp;dir=".urlencode(str_replace("\\","/",dirname($chck_ftp_pwd)))."\">..</a><br />\n"; }
$dirnum = count($dlist); $diri = 0;
while($diri < $dirnum) {
	echo "<a href=\"ftp.php?act=list&amp;dir=".urlencode($myftpdir.$dlist[$diri]['name'])."\">".$dlist[$diri]['name']."</a><br />\n"; ++$diri; }
$filenum = count($flist); $filei = 0;
while($filei < $filenum) {
