...
if ($act == "validate") {
    ...
    if ($bartype == "upca" && strlen($barcode) > 12) {
        preg_match("/^(\d{12})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($bartype == "upca" && validate_upca($barcode, false) === true) {
        echo $barcode." is valid.\n";
    }
    if ($bartype == "upca" && validate_upca($barcode, false) === false) {
        echo $barcode." is invalid.\n";
    }
    if ($bartype == "upce" && strlen($barcode) > 8) {
        preg_match("/^(\d{8})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($bartype == "upce" && validate_upce($barcode, false) === true) {
        echo $barcode." is valid.\n";
    }
    if ($bartype == "upce" && validate_upce($barcode, false) === false) {
        echo $barcode." is invalid.\n";
    }
    if ($bartype == "ean13" && strlen($barcode) > 13) {
        preg_match("/^(\d{13})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($bartype == "ean13" && validate_ean13($barcode, false) === true) {
        echo $barcode." is valid.\n";
    }
    if ($bartype == "ean13" && validate_ean13($barcode, false) === false) {
        echo $barcode." is invalid.\n";
    }
    ...
    if ($bartype == "ean8" && validate_ean8($barcode, false) === false) {
        echo $barcode." is invalid.\n";
    }
    if ($bartype == "itf14" && strlen($barcode) > 14) {
        preg_match("/^(\d{14})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($bartype == "itf14" && validate_itf14($barcode, false) === true) {
        echo $barcode." is valid.\n";
    }
    if ($bartype == "itf14" && validate_itf14($barcode, false) === false) {
        echo $barcode." is invalid.\n";
    }
}
if ($act == "check") {
    ...
    if ($bartype == "upca") {
        echo fix_upca_checksum($barcode, true)."\n";
    }
    if ($bartype == "upce") {
        echo fix_upce_checksum($barcode, true)."\n";
    }
    if ($bartype == "ean13") {
        echo fix_ean13_checksum($barcode, true)."\n";
    }
    if ($bartype == "ean8") {
        echo fix_ean8_checksum($barcode, true)."\n";
    }
    if ($bartype == "itf14") {
        echo fix_itf14_checksum($barcode, true)."\n";
    }
}
if ($act == "create") {
    if (!isset($argv[2])) {
    ...
    if (preg_match("/([0-9]+)([ |\|]{1})([0-9]{2})$/", $barcode, $upc_pieces)) {
        $barcode = $upc_pieces[1];
        $supplement = $upc_pieces[3];
    }
    ...
    if (strlen($supplement) != 2 && strlen($supplement) != 5) {
        $supplement = null;
    }
    if ($bartype == "ean13" && validate_ean13($barcode, false) === true) {
        echo "Creating Barcode: ".$barcode." to file ".$filename."\n";
...
if ($act == "convert") {
    ...
    if ($confrom == "upca" && $conto == "upce" && validate_upca($barcode, false) === true) {
        echo convert_upca_to_upce($barcode)."\n";
    }
    if ($confrom == "upca" && $conto == "ean13" && validate_upca($barcode, false) === true) {
        echo convert_upca_to_ean13($barcode)."\n";
    }
    if ($confrom == "upca" && $conto == "itf14" && validate_upca($barcode, false) === true) {
        echo convert_upca_to_itf14($barcode)."\n";
    }
    if ($confrom == "upce" && strlen($barcode) > 8) {
        preg_match("/^(\d{8})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($confrom == "upce" && $conto == "upca" && validate_upce($barcode, false) === true) {
        echo convert_upce_to_upca($barcode)."\n";
    }
    if ($confrom == "upce" && $conto == "ean13" && validate_upce($barcode, false) === true) {
        echo convert_upce_to_ean13($barcode)."\n";
    }
    if ($confrom == "upce" && $conto == "itf14" && validate_upce($barcode, false) === true) {
        echo convert_upce_to_itf14($barcode)."\n";
    }
    if ($confrom == "ean13" && strlen($barcode) > 13) {
        preg_match("/^(\d{13})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($confrom == "ean13" && $conto == "upca" && validate_ean13($barcode, false) === true) {
        echo convert_ean13_to_upca($barcode)."\n";
    }
    if ($confrom == "ean13" && $conto == "upce" && validate_ean13($barcode, false) === true) {
        echo convert_ean13_to_upce($barcode)."\n";
    }
    if ($confrom == "ean13" && $conto == "itf14" && validate_ean13($barcode, false) === true) {
        echo convert_ean13_to_itf14($barcode)."\n";
    }
    if ($confrom == "itf14" && strlen($barcode) > 14) {
        preg_match("/^(\d{14})/", $barcode, $fix_matches);
        $barcode = $fix_matches[1];
    }
    if ($confrom == "itf14" && $conto == "upca" && validate_itf14($barcode, false) === true) {
        echo convert_itf14_to_upca($barcode)."\n";
    }
    if ($confrom == "itf14" && $conto == "upce" && validate_itf14($barcode, false) === true) {
        echo convert_itf14_to_upce($barcode)."\n";
    }
    if ($confrom == "itf14" && $conto == "ean13" && validate_itf14($barcode, false) === true) {
        echo convert_itf14_to_ean13($barcode)."\n";
    }
}
if ($act == "country") {
    if (!isset($argv[2])) {
