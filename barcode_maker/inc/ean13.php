...
function create_ean13($upc, $imgtype = "png", $outputimage = true, $resize = 1, $resizetype = "resize", $outfile = null, $hidecd = false)
{
    ...
    if (isset($supplement) && !is_numeric($supplement)) {
        return false;
    }
    if (strlen($upc) == 8) {
    ...
    if (strlen($upc) == 12 && validate_upca($upc) === true) {
        $upc = "0".$upc;
    }
    if (strlen($upc) == 12 && validate_upca($upc) === false) {
    ...
    if (validate_ean13($upc) === false) {
        preg_match("/^(\d{12})/", $upc, $pre_matches);
    ...
    $addonsize = 0;
    if (strlen($supplement) == 2) {
        $addonsize = 29;
    }
    if (strlen($supplement) == 5) {
        $addonsize = 56;
    }
