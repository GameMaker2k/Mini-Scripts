...
function convert_ean13_to_upca($upc)
{
    ...
    if (strlen($upc) > 13 || strlen($upc) < 13) {
        return false;
    }
    if (validate_ean13($upc) === false) {
        return false;
...
function convert_itf14_to_ean13($upc)
{
    ...
    if (strlen($upc) > 14 || strlen($upc) < 14) {
        return false;
    }
...
function convert_upca_to_upce($upc)
{
    ...
    if (strlen($upc) > 12 || strlen($upc) < 12) {
        return false;
    }
    if (validate_upca($upc) === false) {
        return false;
    }
    if (!preg_match("/0(\d{11})/", $upc)) {
        return false;
    }
    $upce = null;
...
function print_isbn10($upc)
{
    ...
    if (strlen($upc) > 10 || strlen($upc) < 10) {
        return false;
    }
...
{
    if (strlen($price) > 5) {
        if (preg_match("/^(\d{5})/", $code, $code_matches)) {
            $code = $code_matches[1];
        }
    }
...
{
    if ($numbersystem != 5 && $numbersystem != 9) {
        $numbersystem = 5;
    }
    if (strlen($upc) > 5) {
        preg_match("/^(\d{5})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
    if (strlen($upc) > 3) {
        preg_match("/^(\d{3})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
    if (strlen($upc) > 2) {
        preg_match("/^(\d{2})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
