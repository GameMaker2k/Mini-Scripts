...
{
    if (strlen($upc) > 11) {
        preg_match("/^(\d{11})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
...
function validate_itf14($upc, $return_check = false)
{
    ...
    if (strlen($upc) > 14) {
        preg_match("/^(\d{14})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
    if (strlen($upc) > 14 || strlen($upc) < 13) {
        return false;
    }
    if (strlen($upc) == 13) {
        preg_match("/^(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})/", $upc, $upc_matches);
...
function validate_ean8($upc, $return_check = false)
{
    ...
    if (strlen($upc) > 8) {
        preg_match("/^(\d{8})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
    if (strlen($upc) > 8 || strlen($upc) < 7) {
        return false;
    }
    if (strlen($upc) == 7) {
...
{
    if (strlen($upc) > 7) {
        preg_match("/^(\d{7})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
...
function validate_issn8($upc, $return_check = false)
{
    ...
    if (strlen($upc) > 8) {
        preg_match("/^(\d{8})/", $upc, $fix_matches);
        $upc = $fix_matches[1].$fix_matches[2];
    }
    if (strlen($upc) > 8 || strlen($upc) < 7) {
        return false;
    }
...
function validate_isbn10($upc, $return_check = false)
{
    ...
    if (strlen($upc) > 10) {
        preg_match("/^(\d{9})(\d{1}|X{1})/", $upc, $fix_matches);
        $upc = $fix_matches[1].$fix_matches[2];
    }
    if (strlen($upc) > 10 || strlen($upc) < 9) {
        return false;
    }
    if (strlen($upc) == 9) {
...
function fix_isbn10_checksum($upc)
{
    ...
    $upc = str_replace(" ", "", $upc);
    if (strlen($upc) > 9) {
        preg_match("/^(\d{9})/", $upc, $fix_matches);
        $upc = $fix_matches[1];
    }
...
function validate_ismn10($upc, $return_check = false)
{
    ...
    if (strlen($upc) > 9) {
        preg_match("/^(\d{8})(\d{1})/", $upc, $fix_matches);
        $upc = $fix_matches[1].$fix_matches[2];
    }
    if (strlen($upc) > 9 || strlen($upc) < 8) {
        return false;
    }
    if (strlen($upc) == 8) {
        preg_match("/^(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})(\d{1})/", $upc, $upc_matches);
...
function get_vw_price_checksum($price, $return_check = false)
{
    ...
    if (strlen($price) > 5) {
        if (preg_match("/^(\d{5})/", $price, $price_matches)) {
...
{
    if (strlen($price) == 5) {
        preg_match("/^(\d{1})(\d{4})/", $price, $fix_matches);
        $price = $fix_matches[2];
    }
    if (strlen($price) > 4) {
        preg_match("/^(\d{4})/", $price, $fix_matches);
        $price = $fix_matches[1];
    }
